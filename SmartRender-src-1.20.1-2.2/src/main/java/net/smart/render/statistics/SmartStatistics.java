...
public class SmartStatistics extends SmartStatisticsContext
{
	...
	public SmartStatistics(Player sp)
	{
		this.sp = sp;
	}
	...
	public void calculateAllStats(boolean remote)
	{
		...
		//   posX/posY/posZ -> getX()/getY()/getZ(); prevPosX/prevPosY/prevPosZ -> xo/yo/zo
		double diffX = sp.getX() - sp.xo;
		double diffY = sp.getY() - sp.yo;
		double diffZ = sp.getZ() - sp.zo;
		...
		data.horizontal.calculate(Mth.sqrt((float)(diffX * diffX + diffZ * diffZ)));
		data.vertical.calculate((float)Math.abs(diffY));
		tickDistance = data.all.calculate(Mth.sqrt((float)(diffX * diffX + diffY * diffY + diffZ * diffZ)));
	...
	// 1.8.9 -> 1.20.1 PORT NOTE:
	//   prevLimbSwingAmount -> walkAnimation.speedOld; limbSwingAmount -> walkAnimation.speed; limbSwing -> walkAnimation.position
	//   (the WalkAnimationState fields are exposed via the Smart Render access transformer)
	public float getHorizontalPrevLegYaw() { return ((net.smart.render.mixin.WalkAnimationStateAccessor)sp.walkAnimation).smartrender$getSpeedOld(); }
	public float getHorizontalLegYaw() { return ((net.smart.render.mixin.WalkAnimationStateAccessor)sp.walkAnimation).smartrender$getSpeed(); }
	public float getHorizontalTotal() { return ((net.smart.render.mixin.WalkAnimationStateAccessor)sp.walkAnimation).smartrender$getPosition(); }

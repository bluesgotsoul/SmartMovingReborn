...
public class SmartMovingModel extends SmartRenderContext
{
	...
	private void setRotationAngles(float totalHorizontalDistance, float currentHorizontalSpeed, float totalTime, float viewHorizontalAngelOffset, float viewVerticalAngelOffset, float factor)
	{
		...
		if(isRopeSliding)
		{
			...
			bipedTorso.rotateAngleX = Sixteenth + Sixtyfourth * Mth.cos(time);
			...
			bipedLeftLeg.rotateAngleX = Sixtyfourth * Mth.cos(time + Quarter);
			bipedRightLeg.rotateAngleX = Sixtyfourth * Mth.cos(time - Quarter);
		}
		else if(isClimb || isCrawlClimb)
		{
			...
			bipedRightArm.rotateAngleX = Mth.cos(totalVerticalDistance * handsFrequenceUpFactor + Half) * verticalSpeed * handsDistanceUpFactor + handsDistanceUpOffset;
			bipedLeftArm.rotateAngleX = Mth.cos(totalVerticalDistance * handsFrequenceUpFactor) * verticalSpeed * handsDistanceUpFactor + handsDistanceUpOffset;

			bipedRightArm.rotateAngleY = Mth.cos(totalHorizontalDistance * handsFrequenceSideFactor + Quarter) * horizontalSpeed * handsDistanceSideFactor + handsDistanceSideOffset;
			bipedLeftArm.rotateAngleY = Mth.cos(totalHorizontalDistance * handsFrequenceSideFactor) * horizontalSpeed * handsDistanceSideFactor + handsDistanceSideOffset;
			...
			if(isHandsVineClimbing)
			{
				...
				setArmScales(Math.abs(Mth.cos(bipedRightArm.rotateAngleX)), Math.abs(Mth.cos(bipedLeftArm.rotateAngleX)));
			}

			if(!isFeetVineClimbing)
			{
				bipedRightLeg.rotateAngleX = Mth.cos(totalVerticalDistance * feetFrequenceUpFactor) * feetDistanceUpFactor * verticalSpeed + feetDistanceUpOffset;
				bipedLeftLeg.rotateAngleX = Mth.cos(totalVerticalDistance * feetFrequenceUpFactor + Half) * feetDistanceUpFactor * verticalSpeed + feetDistanceUpOffset;
			}

			bipedRightLeg.rotateAngleZ = -(Mth.cos(totalHorizontalDistance * feetFrequenceSideFactor) - 1.0F) * horizontalSpeed * feetDistanceSideFactor + feetDistanceSideOffset;
			bipedLeftLeg.rotateAngleZ = -(Mth.cos(totalHorizontalDistance * feetFrequenceSideFactor + Quarter) + 1.0F) * horizontalSpeed * feetDistanceSideFactor + feetDistanceSideOffset;

			if(isFeetVineClimbing)
			{
				float total = (Mth.cos(totalDistance + Half) + 1) * Thirtytwoth + Sixteenth;
				bipedRightLeg.rotateAngleX = -total;
				bipedLeftLeg.rotateAngleX = -total;

				float difference = Math.max(0, Mth.cos(totalDistance - Quarter)) * Sixtyfourth;
				bipedLeftLeg.rotateAngleZ += -difference;
				bipedRightLeg.rotateAngleZ += difference;

				setLegScales(Math.abs(Mth.cos(bipedRightLeg.rotateAngleX)), Math.abs(Mth.cos(bipedLeftLeg.rotateAngleX)));
			}
		...
		else if(isCeilingClimb)
		{
			...
			bipedLeftArm.rotateAngleX = (Mth.cos(distance) * 0.52F + Half) * walkFactor + Half * standFactor;
			bipedRightArm.rotateAngleX = (Mth.cos(distance + Half) * 0.52F - Half) * walkFactor - Half * standFactor;

			bipedLeftLeg.rotateAngleX = -Mth.cos(distance) * 0.12F * walkFactor;
			bipedRightLeg.rotateAngleX = -Mth.cos(distance + Half) * 0.32F * walkFactor;

			float rotateY = Mth.cos(distance) * 0.44F * walkFactor;
		...
		else if(isSwim)
		{
			...
			bipedHead.rotateAngleY = Mth.cos(distance / 2.0F - Quarter) * walkFactor;
			...
			bipedBreast.rotateAngleY = bipedBody.rotateAngleY = Mth.cos(distance / 2.0F - Quarter) * walkFactor;

			bipedRightArm.rotationOrder = ModelRotationRenderer.YZX;
			bipedLeftArm.rotationOrder = ModelRotationRenderer.YZX;

			bipedRightArm.rotateAngleZ = Quarter + Eighth + Mth.cos(totalTime * 0.1F) * standSneakFactor * 0.8F;
			bipedLeftArm.rotateAngleZ = -Quarter - Eighth - Mth.cos(totalTime * 0.1F) * standSneakFactor * 0.8F;
			...
			bipedRightLeg.rotateAngleX = Mth.cos(distance) * 0.52264464F * walkFactor;
			bipedLeftLeg.rotateAngleX = Mth.cos(distance + Half) * 0.52264464F * walkFactor;

			float rotateFeetAngleZ = Sixteenth * standSneakFactor + Mth.cos(totalTime * 0.1F) * 0.4F * (standFactor - sneakFactor);
			bipedRightLeg.rotateAngleZ = rotateFeetAngleZ;
			bipedLeftLeg.rotateAngleZ = -rotateFeetAngleZ;

			if(scaleLegType != NoScaleStart)
				setLegScales(
					1F + (Mth.cos(totalTime * 0.1F + Quarter) - 1F) * 0.15F * sneakFactor,
					1F + (Mth.cos(totalTime * 0.1F + Quarter) - 1F) * 0.15F * sneakFactor);
		...
		else if(isDive)
		{
			...
			bipedRightLeg.rotateAngleZ = (Mth.cos(distance) + 1F) * 0.52264464F * walkFactor + Sixteenth * standFactor;
			bipedLeftLeg.rotateAngleZ = (Mth.cos(distance + Half) - 1F) * 0.52264464F * walkFactor - Sixteenth * standFactor;

			if(scaleLegType != NoScaleStart)
				setLegScales(
					1F + (Mth.cos(distance - Quarter) - 1F) * 0.25F * walkFactor,
					1F + (Mth.cos(distance - Quarter) - 1F) * 0.25F * walkFactor);

			bipedRightArm.rotateAngleZ = (Mth.cos(distance + Half) * 0.52264464F * 2.5F + Quarter) * walkFactor + (Quarter + Eighth) * standFactor;
			bipedLeftArm.rotateAngleZ = (Mth.cos(distance) * 0.52264464F * 2.5F - Quarter) * walkFactor - (Quarter + Eighth) * standFactor;

			if(scaleArmType != NoScaleStart)
				setArmScales(
					1F + (Mth.cos(distance + Quarter) - 1F) * 0.15F * walkFactor,
					1F + (Mth.cos(distance + Quarter) - 1F) * 0.15F * walkFactor);
		}
		else if(isCrawl)
		{
			...
			bipedTorso.rotateAngleZ = Mth.cos(distance + Quarter) * Sixtyfourth * walkFactor;
			bipedBody.rotateAngleY = Mth.cos(distance + Half) * Sixtyfourth * walkFactor;

			bipedRightLeg.rotateAngleX = (Mth.cos(distance - Quarter) * Sixtyfourth + Thirtytwoth) * walkFactor + Thirtytwoth * standFactor;
			bipedLeftLeg.rotateAngleX = (Mth.cos(distance - Half - Quarter) * Sixtyfourth + Thirtytwoth) * walkFactor + Thirtytwoth * standFactor;

			bipedRightLeg.rotateAngleZ = (Mth.cos(distance - Quarter) + 1F) * 0.25F * walkFactor + Thirtytwoth * standFactor;
			bipedLeftLeg.rotateAngleZ = (Mth.cos(distance - Quarter) - 1F) * 0.25F * walkFactor - Thirtytwoth * standFactor;

			if(scaleLegType != NoScaleStart)
				setLegScales(
						1F + (Mth.cos(distance + Quarter - Quarter) - 1F) * 0.25F * walkFactor,
						1F + (Mth.cos(distance - Quarter - Quarter) - 1F) * 0.25F * walkFactor);
			...
			bipedRightArm.rotateAngleZ = ((Mth.cos(distance + Half)) * Sixtyfourth + Thirtytwoth)* walkFactor + Sixteenth * standFactor;
			bipedLeftArm.rotateAngleZ = ((Mth.cos(distance + Half)) * Sixtyfourth - Thirtytwoth) * walkFactor - Sixteenth * standFactor;
		...
		else if(isSlide)
		{
			...
			bipedBody.rotateAngleX = Mth.cos(distance - Eighth) * Sixtyfourth * walkFactor;
			bipedBody.rotateAngleY = Mth.cos(distance + Eighth) * Sixtyfourth * walkFactor;

			bipedRightLeg.rotateAngleX = Mth.cos(distance + Half) * Sixtyfourth * walkFactor + Sixtyfourth;
			bipedLeftLeg.rotateAngleX = Mth.cos(distance + Quarter) * Sixtyfourth * walkFactor + Sixtyfourth;
			...
			bipedRightArm.rotateAngleX = Mth.cos(distance + Quarter) * Sixtyfourth * walkFactor + Half - Sixtyfourth;
			bipedLeftArm.rotateAngleX = Mth.cos(distance - Half) * Sixtyfourth * walkFactor + Half - Sixtyfourth;
		...
		else if(isFlying)
		{
			...
			bipedRightArm.rotateAngleY = (Mth.cos(time) * Sixteenth) * standFactor;
			bipedLeftArm.rotateAngleY = (Mth.cos(time) * Sixteenth) * standFactor;

			bipedRightArm.rotateAngleZ = (Mth.cos(distance + Half) * Sixtyfourth + (Half - Sixteenth)) * walkFactor + Quarter * standFactor;
			bipedLeftArm.rotateAngleZ = (Mth.cos(distance) * Sixtyfourth - (Half - Sixteenth)) * walkFactor - Quarter * standFactor;

			bipedRightLeg.rotateAngleX = Mth.cos(distance) * Sixtyfourth * walkFactor + Mth.cos(time + Half) * Sixtyfourth * standFactor;
			bipedLeftLeg.rotateAngleX = Mth.cos(distance + Half) * Sixtyfourth * walkFactor + Mth.cos(time) * Sixtyfourth * standFactor;
		...
		else if(isHeadJump)
		{
			...
			if(overGroundBlock != null && overGroundBlock.isSolid())
		...
		else if(isFalling)
		{
			...
			bipedRightArm.rotateAngleY = (Mth.cos(distance + Quarter) * Eighth);
			bipedLeftArm.rotateAngleY = (Mth.cos(distance + Quarter) * Eighth);

			bipedRightArm.rotateAngleZ = (Mth.cos(distance) * Eighth + Quarter);
			bipedLeftArm.rotateAngleZ = (Mth.cos(distance) * Eighth - Quarter);

			bipedRightLeg.rotateAngleX = (Mth.cos(distance + Half + Quarter) * Sixteenth + Thirtytwoth);
			bipedLeftLeg.rotateAngleX = (Mth.cos(distance + Quarter) * Sixteenth + Thirtytwoth);

			bipedRightLeg.rotateAngleZ = (Mth.cos(distance) * Sixteenth + Thirtytwoth);
			bipedLeftLeg.rotateAngleZ = (Mth.cos(distance) * Sixteenth - Thirtytwoth);
		}
		else
			isStandard = true;
	}
	...
	public BlockState overGroundBlock;

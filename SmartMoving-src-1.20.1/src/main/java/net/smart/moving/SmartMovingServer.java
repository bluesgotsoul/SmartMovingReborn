...
// 1.8.9 -> 1.20.1 adaptations:
//   net.minecraftforge.fml.common.FMLLog            -> SLF4J logger (LogUtils.getLogger())
//   net.minecraftforge.fml.common.network.internal  (FMLProxyPacket) -> gone; the incoming state
//       packet is re-broadcast to tracked players by re-serializing the identical (entityId,state)
//       blob via SmartMovingPacketStream (byte-identical to the bytes the client sent).
...
public class SmartMovingServer
{
	...
	// 1.8.9: processStatePacket(FMLProxyPacket packet, long state) re-broadcast the very same
	// FMLProxyPacket to tracked players. The entityId is now passed explicitly so the identical
	// state blob can be rebuilt and re-broadcast as a byte[] (byte-for-byte equal to the original).
	public void processStatePacket(int entityId, long state)
	{
		...
		mp.sendPacketToTrackedPlayers(SmartMovingPacketStream.serializeState(entityId, state));
	}
	...
	public static void initialize(File optionsPath, int gameType, SmartMovingConfig config)
	{
		...
		LOGGER.info(SmartMovingInfo.ModComMessage);
	}
	...
	public void afterOnLivingUpdate()
	{
		...
		AABB box = mp.expandBox(mp.getBox(), 1, offset, 1);

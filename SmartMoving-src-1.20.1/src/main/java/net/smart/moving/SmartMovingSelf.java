// Smart Moving - SmartMovingSelf
//
// Faithful 1.20.1 port of the original 1.8.9 SmartMovingSelf (3199 lines).
//
// This file is the local-player physics owner. It is delivered here as a
// SKELETON that already carries the complete field surface and the full
// public / @Override method surface that the rest of the mod (client
// player-base glue, SmartMovingContext, SmartMovingComm, render layer) calls
// against. Self-contained accessors (exhaustion / jump-charge family, jump(),
// isJumping, NBT save, activate-block sneak override) are implemented 1:1.
// The heavy movement math (swimming, climbing, wall jumping, land motion,
// height handling, ...) has since been ported 1:1 in the dedicated physics-logic
// pass; the private helper methods carry their real bodies and their signatures
// stay faithful to 1.8.9.
//
// 1.8.9 -> 1.20.1 surface notes:
//   EntityPlayer            -> net.minecraft.world.entity.player.Player (base 'sp')
//   NBTTagCompound          -> net.minecraft.nbt.CompoundTag
//   EnumStatus sleepInBedAt -> Either<Player.BedSleepingProblem, Unit> sleepInBedAt(BlockPos)
//   getCompoundTag/hasKey/setBoolean -> getCompound/contains/putBoolean
//   sp.capabilities.isFlying -> sp.getAbilities().flying
//   sp.getAIMoveSpeed()     -> sp.getSpeed()
//   Block (metadata int kept as-is for parity; metadata removed in 1.20.1)
...
public class SmartMovingSelf extends SmartMoving implements ISmartMovingSelf {
	...
	// ----------------------------------------------------------------------
	// SmartMoving abstract animation hooks
	// ----------------------------------------------------------------------
	...
	public void beforeOnUpdate() {
		...
		wasCollidedHorizontally = sp.horizontalCollision;
		...
		if (sp.level().isClientSide && updateCounter < 10) {
			...
			// This supersedes the 1.8.9 tick-scan via GuiNewChat.chatLines.
			updateCounter++;
		}
	}
	...
	private void superMoveEntityWithHeading(float moveStrafing, float moveForward) {
		...
		double diffX = sp.getX() - d_S;
		double diffY = sp.getY() - d1_S;
		double diffZ = sp.getZ() - d2_S;

		addMovementStat(diffX, diffY, diffZ);
	...
	// ----------------------------------------------------------------------
	// Movement sub-handlers (signatures faithful to 1.8.9; heavy bodies are
	...
	private boolean handleSwimming(float moveForward, float moveStrafing, float speedFactor, boolean wasSwimming,
		...
		if (handleSwimming) {
			...
			if (!useStandard) {
				...
				boolean diveDown = esp.input.shiftKeyDown && Config._diveDownOnSneak.value;
			...
			if (useStandard) {
				...
				Vec3 m = sp.getDeltaMovement();
				double nmX = m.x * 0.80000001192092896D;
				double nmY = m.y * 0.80000001192092896D;
				double nmZ = m.z * 0.80000001192092896D;
				nmY -= 0.02D;
	...
	private boolean handleLava(float moveForward, float moveStrafing, boolean handledSwimming,
		...
		if (handleLava) {
			...
			double d1 = sp.getY();
			superMoveFlying(moveStrafing, moveForward, 0.02F);
			sp.move(MoverType.SELF, sp.getDeltaMovement());
			Vec3 m = sp.getDeltaMovement();
			double nmX = m.x * 0.5D;
			double nmY = m.y * 0.5D;
	...
	private boolean handleAlternativeFlying(float moveForward, float moveStrafing, float speedFactor,
		...
		if (handleAlternativeFlying) {
			...
			Vec3 m = sp.getDeltaMovement();
			sp.setDeltaMovement(m.x * HorizontalAirDamping, m.y * HorizontalAirDamping, m.z * HorizontalAirDamping);
		}
				...
				if (isSliding) {
					...
					if (moveStrafing != 0 && Config._slideControlDegrees.value > 0) {
						...
						if (!Double.isNaN(angle)) {
							...
							double hMotion = Math.sqrt(mv.x * mv.x + mv.z * mv.z);
	...
	public boolean tryJump(int type, Boolean inWaterOrNull, Boolean isRunningOrNull, Float angle) {
		...
		if (enabled) {
			...
			Vec3 dm = sp.getDeltaMovement();
			double mX = dm.x, mY = dm.y, mZ = dm.z;
			...
			if (angle != null) {
				...
				mX = getJumpMoving(jumpMotionX, moveX, reset, horizontal, horizontalJumpFactor);
				mZ = getJumpMoving(jumpMotionZ, moveZ, reset, horizontal, horizontalJumpFactor);
		...
		if (couldClimbCeiling && exhaustionAllowsClimbCeiling) {
			...
			if (topCeilingClimbing || bottomCeilingClimbing) {
				...
				if (jgap < 1.9 && actuallySolidHeight < jd + 0.5) {
					...
					sp.setDeltaMovement(sp.getDeltaMovement().x, ceilingMotionY, sp.getDeltaMovement().z);
					sp.fallDistance = 0.0F;
	...
	private void setLandMotions(float horizontalDamping) {
		...
		mX *= horizontalDamping;
		mZ *= horizontalDamping;
		sp.setDeltaMovement(mX, mY, mZ);
	}
	...
	private void handleLand(float moveForward, float moveStrafing, float speedFactor, boolean handledSwimming,
		...
		if (!handledSwimming && !handledLava && !handledAlternativeFlying) {
			...
			move(sp.getDeltaMovement().x, sp.getDeltaMovement().y, sp.getDeltaMovement().z, crawlingThroughWeb);
	...
	public void handleJumping() {
		...
		jumpMotionX = sp.getDeltaMovement().x;
	...
	public void afterMoveEntity(double d, double d1, double d2) {
		...
		double d10 = sp.getX() - beforeMoveEntityPosX;
	...
	public void updateEntityActionState(boolean startSleeping) {
		...
		boolean isLevitating = sp.getAbilities().flying && !isFlying;
		boolean isRunning = isRunning();
		...
		net.minecraft.client.Options gameSettings = minecraft.options;

		forwardButton.update(gameSettings.keyUp);
		leftButton.update(gameSettings.keyLeft);
		rightButton.update(gameSettings.keyRight);
		backButton.update(gameSettings.keyDown);
		jumpButton.update(esp.input.jumping);
		sprintButton.update(gameSettings.keySprint);
		sneakButton.update(esp.input.shiftKeyDown);

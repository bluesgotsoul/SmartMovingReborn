...
public abstract class SmartMovingBase extends SmartMovingContext {
	...
	protected void moveFlying(float moveUpward, float moveStrafing, float moveForward, float speedFactor,
		...
		float diffMotionY = Mth.sqrt(diffMotionXForward * diffMotionXForward + diffMotionZForward * diffMotionZForward)
				* divingVerticalFactor + moveUpward;
		float diffMotionZ = diffMotionZForward * divingHorizontalFactor + diffMotionZStrafing;

		float total = Mth
				.sqrt(Mth.sqrt(diffMotionX * diffMotionX + diffMotionZ * diffMotionZ) + diffMotionY * diffMotionY);
		if (total > 0.01F) {
	...
	protected float getNormalWaterBorder(int i, int j, int k) {
		...
		// Падающая вода (старое level >= 8) = полный блок.
		if (fluid.hasProperty(FlowingFluid.FALLING) && fluid.getValue(FlowingFluid.FALLING))
			return 1F;

		int amount = fluid.getAmount(); // 1..8, 8 = источник / полный
		if (amount >= 8) {
			// источник (старое level == 0): чуть заниженная поверхность, если сверху воздух
			if (getState(i, j + 1, k).isAir())
				return 0.8875F;
			return 1F;
		}
		return amount / 8F; // течение: эквивалент (8 - level) / 8F
	}
					...
					for (Orientation direction : facedOnlyTo != null ? facedOnlyTo : Orientation.Orthogonals) {
						...
						if (direction != localLadderOrientation) {
							...
							if (Orientation.isKnownLadder(remoteState)) {
								Orientation remoteLadderOrientation = Orientation.getKnownLadderOrientation(sp.level(),
										i + direction._i, j, k + direction._k);
				...
				if (vine && Orientation.isVine(state))
					if (facedOnlyTo == null)
						...
						while (iterator.hasNext()) {
							...
							if (climbOrientation.hasVineOrientation(sp.level(), i, j, k)
									&& climbOrientation.isRemoteSolid(sp.level(), i, j, k)) {
								result++;
								break;
							}
						}
	...
	private boolean isOpenBlockSpace(BlockPos pos, boolean top) {
		return !sp.level().getBlockState(pos).isRedstoneConductor(sp.level(), pos)
				&& (!top || !sp.level().getBlockState(pos.above()).isRedstoneConductor(sp.level(), pos.above()));
	}

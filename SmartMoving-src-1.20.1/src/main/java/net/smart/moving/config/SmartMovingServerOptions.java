...
// 1.8.9 used net.minecraftforge.fml.common.FMLLog for server-side logging. In 1.20.1 that class
// is gone; we use the standard SLF4J logger (same one Forge/Mojang expose via LogUtils).
	...
	private static void logConfigState(SmartMovingConfig config, String username, boolean reconfig)
	{
		...
		if(config._globalConfig.value)
		{
			...
			if(config.enabled)
			{
				...
				if(currentKey == null)
					LOGGER.info(message + "default server configuration" + postfix);
				else
				{
					...
					if(configName.isEmpty())
						LOGGER.info(message + "with key \"" + currentKey + "\"" + postfix);
					else
						LOGGER.info(message + "\"" + configName + "\"" + postfix);
				}
			}
			else
				LOGGER.info(message + "disabled" + postfix);
		}
		else

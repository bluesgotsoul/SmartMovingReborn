...
{
	// 1.8.9: void sendPacketToTrackedPlayers(FMLProxyPacket packet);
	// 1.20.1: the transport carries a raw byte[] blob dispatched through SmartMovingNetwork
	// (PacketDistributor.TRACKING_ENTITY), so the wrapped FMLProxyPacket is replaced by byte[].
	void sendPacketToTrackedPlayers(byte[] data);
	...
	AABB expandBox(AABB box, double x, double y, double z);

	List<?> getEntitiesExcludingPlayer(AABB box);

...
public class Value<T>
{
	...
	// 1.8.9 -> 1.20.1: the original key <-> name translation reflected LWJGL2's
	// org.lwjgl.input.Keyboard / Mouse. That library is gone in 1.20.1; the translation now
	// lives in net.smart.utilities.KeyNames (GLFW / InputConstants), preserving the original
	// contract (mouse buttons carry a -100 offset).
	public static String toKeyName(Integer keyCode)
	{
		return KeyNames.toKeyName(keyCode);
	}

	private static Integer toKeyCode(String keyName)
	{
		return KeyNames.toKeyCode(keyName);
	}
}
